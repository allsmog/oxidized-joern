...
object PyAstGenRunner {

  private object astGenMetaData
      extends AstGenProgramMetaData(
        name = "pyastgen",
        configPrefix = "pysrc2cpg",
        binEnvVar = Option("PYASTGEN_BIN"),
        versionFlag = "-version"
      )
}

class PyAstGenRunner(config: Py2CpgOnFileSystemConfig) extends AstGenRunner(PyAstGenRunner.astGenMetaData, config) {
