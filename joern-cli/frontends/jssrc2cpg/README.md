...
oxidized replacement track for the JavaScript/TypeScript frontend and emits the
same Babel-shaped JSON contract consumed by the Scala frontend. For TypeScript
inputs it can also emit `.typemap` sidecars used by type recovery.
...
Run the Scala frontend suite against the locally built Rust binary with:
