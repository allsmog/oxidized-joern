...
final class OxidizedAstCreator(document: JavaAstDocument, config: Config)
  ...
  private def astForIfStatement(node: JavaAstNode): Ast = {
    ...
    ifThenElseAst(controlStructureNode(node, ControlStructureTypes.IF, node.code), condition, consequence, alternative)
  }
  ...
  private def astForForStatement(node: JavaAstNode): Ast = {
    ...
    val ast = withScope(Nil) {
      ...
      forAst(
        controlStructureNode(node, ControlStructureTypes.FOR, declarationHeader(node)),
        Nil,
        initAsts,
        conditionAsts,
        updateAsts,
  ...
  private def astForSwitchExpression(node: JavaAstNode, controlStructureType: String): Ast = {
    ...
    val switchNode = controlStructureNode(node, controlStructureType, s"switch(${unparenthesizedCode(conditionNode)})")
    val bodyAst = childByField(node, "body")
      .map(astForSwitchBody(_, controlStructureType, conditionNode))
      .getOrElse(emptyBlockAst(node))
    switchAst(switchNode, conditionAst, List(bodyAst))
  }
  ...
  private def astsForSwitchPatternEntry(
    ...
    val ifBody = switchGuardExpressionNode(labelNode)
      .map { guardNode =>
        ...
        val guardIf = ifThenElseAst(
          controlStructureNode(guardNode, ControlStructureTypes.IF, s"if (${guardNode.code})"),
          Some(guardAst),
          guardBody,
          None
        )
    ...
    val ifAst = ifThenElseAst(
      controlStructureNode(labelNode, ControlStructureTypes.IF, s"if (${astRootCode(conditionAst)})"),
      Some(conditionAst),
      ifBody,
      None
    )
  ...
  private def astsForTryStatement(node: JavaAstNode): Seq[Ast] = {
    withScope(Nil) {
      ...
      val tryNode = controlStructureNode(node, ControlStructureTypes.TRY, "try")
      ...
      resourceAsts :+ tryCatchAst(tryNode, tryBody, catchAsts, finallyAst)
    }
  }
