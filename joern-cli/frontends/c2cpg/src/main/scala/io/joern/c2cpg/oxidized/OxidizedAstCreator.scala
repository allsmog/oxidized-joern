...
final class OxidizedAstCreator(filename: String, document: OxDocument, config: Config)
  ...
  private def constructorInitializerUnwindAsts(
    ...
    if (asts.nonEmpty && cleanupAsts.nonEmpty) {
      ...
      Seq(tryCatchAst(OxOrigin("try", Option(line)), tryBlock, Seq.empty, None, Option("try")))
    } else {
      asts
    }
  }
  ...
  private def astsForStatement(statement: OxStatement): Seq[Ast] = {
    statement match {
      ...
      case tryStmt: OxTry =>
        ...
        Seq(tryCatchAst(OxOrigin("try", Option(tryStmt.line)), bodyAst, catchAsts, None, Option("try")))
      case ifStmt: OxIf =>
        def ifAsts: Seq[Ast] = {
          ...
          initializerAsts :+
            ifThenElseAst(OxOrigin(ifStmt), Option(conditionAst), thenAst, elseAst, Option(ifStmt.code))
      ...
      case forStmt: OxFor =>
        val (forAst_, initializerDestructors) = inNestedScopeCollectingDestructors {
          ...
          forAst(OxOrigin(forStmt), localAsts, initAsts, conditionAsts, updateAsts, Seq(bodyAst), code = Option(forStmt.code))
            .withChildren(
      ...
      case switchStmt: OxSwitch =>
        val (switchAsts, switchDestructors) = inNestedScopeCollectingDestructors {
          ...
          val switchAst_ = {
            ...
            switchAst(OxOrigin(switchStmt), Option(conditionAst), bodyAsts, code = Option(switchStmt.code))
              .withChildren(conditionHeapConstructors ++ conditionTemporaryCleanup)
          }
  ...
  private def staticLocalInitializationAsts(
    ...
    Option
      .when(initializationAsts.nonEmpty) {
        ...
        Seq(
          Ast(guardLocal),
          ifThenElseAst(OxOrigin(ifCode, Option(local.line)), Option(guardCondition), thenAst, None, code = Option(ifCode))
        )
      }
      .getOrElse(Seq.empty)
  }
  ...
  private def staticLocalDestructorAst(storage: StaticLocalStorage): Seq[Ast] = {
    ...
    Option
      .when(destructorAsts.nonEmpty) {
        ...
        ifThenElseAst(
          OxOrigin(ifCode, Option(storage.local.line)),
