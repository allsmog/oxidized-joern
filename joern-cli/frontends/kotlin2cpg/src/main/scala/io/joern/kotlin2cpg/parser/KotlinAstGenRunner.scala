...
object KotlinAstGenRunner {

  private object astGenMetaData
      extends AstGenProgramMetaData(
        name = "kotlinastgen",
        configPrefix = "kotlin2cpg",
        binEnvVar = Option("KOTLINASTGEN_BIN"),
        versionFlag = "-version"
      )
}

class KotlinAstGenRunner(config: Config) extends AstGenRunner(KotlinAstGenRunner.astGenMetaData, config) {
