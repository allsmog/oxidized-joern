...
#[test]
fn emits_checked_in_scala_ast_model_for_scala_ast_only() {
    let output = Command::cargo_bin("SwiftAstGen")
        .unwrap()
        .arg("--scalaAstOnly")
        .output()
        .unwrap();
    ...
    assert!(stdout.contains("object SwiftNodeSyntax"));
    assert!(stdout.contains("def createSwiftNode"));
    assert!(stdout.contains("Automatically generated by 'SwiftAstGen --scalaAstOnly'"));
}
