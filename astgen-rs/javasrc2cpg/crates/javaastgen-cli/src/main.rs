...
use javaastgen_core::{output_path, parse_file, write_json};
use regex::Regex;
use std::path::{Path, PathBuf};

#[derive(Parser, Debug)]
#[command(name = "javaastgen", disable_version_flag = true)]
struct Args {
...
fn collect_inputs(input: &Path, exclude: Option<&Regex>) -> Result<Vec<PathBuf>> {
    if input.is_file() {
        if is_java_input(input) && !is_excluded(input, exclude) {
            return Ok(vec![input.to_path_buf()]);
        }
        bail!("input file is not a .java file: {}", input.display());
    }
    ...
    for entry in WalkBuilder::new(input).hidden(false).build() {
        let entry = entry?;
        let path = entry.path();
        if path.is_file() && is_java_input(path) && !is_excluded(path, exclude) {
...
fn is_java_input(path: &Path) -> bool {
