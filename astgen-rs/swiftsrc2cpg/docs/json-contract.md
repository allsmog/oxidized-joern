...
This document freezes the standalone Rust `SwiftAstGen` compatibility
...
When `input` is omitted, the current working directory is parsed. Directory
walks skip the default top-level folders (`.*`, `__*`, `test`, `tests`, `spec`,
`specs`). Unsupported
...
form the public envelope:

- `relativeFilePath` — path relative to the input root.
...
Consumers dispatch on `nodeType` (falling back to `tokenKind`) and can access
named children by keypath.
...
- **`name` (keypath label on essentially every node)** — the SwiftSyntax
  ...
  artifact with no tree-sitter equivalent. The differential harness strips it
  from both trees.
