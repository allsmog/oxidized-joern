...
impl<'a> SwiftSyntaxEmitter<'a> {
    ...
    fn enum_case_decl(&self, node: Node<'a>) -> Result<Value> {
        ...
        if names.is_empty() {
            ...
            // Consumers iterate `elements.children`, so an empty list is
    ...
    fn variable_accessor_block(&self, node: Node<'a>) -> Result<Value> {
        match node.kind() {
            ...
            // scrape out of the source. Downstream lowering maps the accessor list
    ...
    /// precise SwiftSyntax node. Downstream lowering turns every `Missing*Syntax`
    ...
    /// text). Downstream lowering maps every `Missing*Syntax` to an empty AST,
