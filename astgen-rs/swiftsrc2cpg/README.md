...
This workspace contains the standalone Rust `SwiftAstGen` implementation.
...
When `input` is omitted, the current working directory is parsed.
Directory walks skip these default top-level folders
...
- `name` (on essentially every node) — the SwiftSyntax child-field/keypath
  ...
  tree-sitter equivalent. It is treated as an intentional, CPG-irrelevant
  divergence and is stripped from both trees by the
