...
| 2. CPGQL | Every standard node-type, property/filter, AST, call-graph, CFG, data-flow, repeat, complex, and execution step in the committed catalog has differential positive, empty, duplicate, regex, and error cases against v4.0.555. Existing compatible queries run unchanged through `cpg query`. | `acceptance/cpgql/catalog.json`; native and Joern result normalizers; zero-diff CI | **In progress.** The native catalog now has 70 executable cases, including unchanged selector/traversal shorthand, recursive AST, boolean filters, range/exact/negative filters, call graph, CFG, dominance, post-dominance, control dependence, richer `repeat`, `reachableBy`, and path-producing `reachableByFlows`; a 33-case real Joern v4.0.555 differential is zero-diff. Annotated dumps, general Scala-lambda execution, every negative/error dimension, and full-catalog differential coverage remain. |
